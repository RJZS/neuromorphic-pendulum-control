...
adaptive_switch_time = 0.1 # When to switch on adaptive control
...
# A:    0.37  0.57  0.68  0.79  0.85  0.91
...
@variables a3(t) a3sat(t) a4(t) a4sat(t)
...
@named first_neur = ODESystem([
    ...
    τs * D(vs)  ~ (v - vs),
    τus * D(vus)  ~ (v - vus),
    gsyn ~ -0.05, #IfElse.ifelse(t > 6, 0.05, -.05),
    a3sat ~ a3,#sigmoid_a3(a3),
    a4sat ~ sigmoid_a4(a4),
])
...
affect = [prev_event ~ latest_event,
            latest_event ~ t,
            latest_freq ~ IfElse.ifelse(2*pi/((latest_event-prev_event)*2) < sat_freq, 2*pi/((latest_event-prev_event)*2), sat_freq)] 
...
@named pendulum = ODESystem([
    ...
    # The above is the equivalent of multiplying the ddq and sin(q) terms
    # by m_scale.
...
a_ref = 0.87 # Desired amplitude
@variables theta(t) error(t) amp_error(t) freq(t) amp(t)
if with_controller
    @parameters k_w = -0.1#-0.02 k_A = 0#-0.1
else
...
@named controller = ODESystem([
    amp_error ~ amp - a_ref,
    error ~ freq - w_ref,
    D(a4) ~ adaptive_on*IfElse.ifelse(abs(error) > 0.01, k_w * error, 0),
    D(a3) ~ adaptive_on*IfElse.ifelse(abs(amp_error) > 0.02, k_A*amp_error, 0),
], t)

full_sys = compose(ODESystem([
        pendulum.v1 ~ first_neur.v,
        pendulum.v3 ~ second_neur.v,
...
init_sys = [first_neur.v => 0, first_neur.vs => 0, 
            ...
            controller.a3 => a3_init,#inv_sigmoid_a3(a3_init),
...
pks, pk_vals = findmaxima(sol[pendulum.angle][i:end])
...
if with_controller
    figure_title = "4_adaptivecontrol_on"
else
    figure_title = "4_adaptivecontrol_off"
end
...
# @save save_string t torque angle freq_error a4 a4sat amp_error a3 a3sat
