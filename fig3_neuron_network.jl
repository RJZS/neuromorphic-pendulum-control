...
config_switch_time = 8 # When to switch the config
init_gsyn = -0.04
...
sol = solve(prob,tstops=[config_switch_time],abstol=1e-11, reltol=1e-9,maxiters=2e6,saveat=5e-3);
...
# NB take care here; the neurons are numbered in the paper 
# differently from in the code.
t = sol.t;  v1 = sol[first_neur.v];  v3 = sol[second_neur.v]
v2 = sol[third_neur.v]; v4 = sol[fourth_neur.v]
torque = sol[pendulum.torque]; switch_time = config_switch_time
save_string = "CDC_paper/figs/neuron_network.jld"
@save save_string t v1 v2 v3 v4 torque switch_time
