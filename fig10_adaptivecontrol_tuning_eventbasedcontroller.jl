...
# To understand the code, please note the following implementation detail:
# The pulses are implemented using the dynamics dx/dt = -k*x. When an appropriate event occurs, the state x is set to 1.
# x > 0.5 is treated as the pulse being on (ie set to 1), and x <= 0.5 is treated as the pulse being off (ie set to 0).
# The pulse duration is therefore the half-life, which is used to set the decay rate k.

with_controller = true # Toggle to switch the adaptive control on or off.

experiment_idx = 4 # Up to 6.
...
adaptive_switch_time = 4 # When to switch on adaptive control.
...
function bounded_ax(u, imin, height)
    if u < imin
        return imin
    elseif u > imin+height
        return imin+height
    else return u
    end
end
function bounded_a3(u)
    bounded_ax(u, 0.9, 1.5)
end
function bounded_a4(u)
    bounded_ax(u, 1.4, 2.5)
end
...
@register_symbolic bounded_a3(u)
@register_symbolic bounded_a4(u)
...
@variables pulse_a4(t) pulse_a3(t) pulse_a4_decay_rate(t) pulse_a3_decay_rate(t)
@variables freq_event_prediction_error(t) amp_event_prediction_error(t)
@variables amp_event_error_sign(t)
...
affect = [prev_event ~ latest_event,
            ...
            freq_event_prediction_error ~ pi/w_ref - (latest_event-prev_event),
            pulse_a4 ~ 1,
            pulse_a4_decay_rate ~ 0.6*log(2)/abs(freq_event_prediction_error),

            ampcontrol_intermediate_var ~ IfElse.ifelse(ampcontrol_intermediate_var == 0, 0, ampcontrol_intermediate_var - 0.5),
            amp_event_last_period ~ ceil(ampcontrol_intermediate_var), # Was there an amplitude event in the last period?

            # If A < A_ref (ie no q=a_ref events).
            amp_event_error_sign ~ -1,
            pulse_a3 ~ IfElse.ifelse(amp_event_last_period > 0, pulse_a3, 1),
            pulse_a3_decay_rate ~ 
                IfElse.ifelse(amp_event_last_period > 0, pulse_a3_decay_rate,
                    3*log(2)/(pi/(2*w_ref))),
            ] 
root_eqs_amp = [d_angle ~ 0] # Measure amplitude of signal
dead_zone = 0 # 0.005 # No dead-zone needed.
...
affect_amp_new = [amp_event_last_period ~ 1,
                ...
                amp_event_prediction_error ~ -(pi/(2*w_ref) - (t-latest_event)),
                pulse_a3 ~ IfElse.ifelse(amp_event_prediction_error > dead_zone, 1, pulse_a3),
                pulse_a3_decay_rate ~ 3*log(2)/abs(amp_event_prediction_error),
                amp_event_error_sign ~ 1, # Amp is too large
                ]
...
@named pendulum = ODESystem([
    ...
    D(prev_event) ~  0.,
    D(latest_freq) ~ 0.,
    D(pulse_a4) ~ -pulse_a4_decay_rate * pulse_a4,
    D(pulse_a4_decay_rate) ~ 0.,
    D(freq_event_prediction_error) ~ 0.,
    ...
    D(out_time) ~ 0.,
    D(back_time) ~ 0.,
    D(pulse_a3) ~ -pulse_a3_decay_rate * pulse_a3,
    D(pulse_a3_decay_rate) ~ 0.,
    D(amp_event_prediction_error) ~ 0.,
    D(amp_event_error_sign) ~ 0.,
], t; continuous_events)

# period_frac = 10 # For old 'dead-zone' setting method.
@variables theta(t) error(t) amp_error(t) freq(t) amp(t) d_a3(t)
@variables rectified_pulse_a4(t) rectified_pulse_a3(t)
if with_controller
    @parameters k_w = -0.05 k_A = -0.5
else
    @parameters k_w = 0 k_A = 0
end
curr_pred_error_gains = prediction_error_gains[experiment_idx,:]
@named controller = ODESystem([
    amp_error ~ amp - a_ref,
    error ~ freq - w_ref,
    # D(a4) ~ adaptive_on*IfElse.ifelse(abs(error) > 0.05, k_w * error, 0),
    rectified_pulse_a4 ~ IfElse.ifelse(pulse_a4 > 0.5, 1, 0),
    rectified_pulse_a3 ~ IfElse.ifelse(pulse_a3 > 0.5, 1, 0),

    D(a4) ~ adaptive_on*rectified_pulse_a4*sign(freq_event_prediction_error)*curr_pred_error_gains[1]*k_w,
    ...
    D(a3) ~ adaptive_on*rectified_pulse_a3*amp_event_error_sign*curr_pred_error_gains[2]*k_A,
], t)
...
full_sys = compose(ODESystem([
        ...
        controller.freq_event_prediction_error ~ pendulum.freq_event_prediction_error,
        controller.amp_event_error_sign ~ pendulum.amp_event_error_sign,
        ...
        controller.pulse_a4 ~ pendulum.pulse_a4,
        controller.pulse_a3 ~ pendulum.pulse_a3,
...
init_sys = [first_neur.v => 0, first_neur.vs => 0, 
            ...
            controller.a3 => a3_init,
            controller.a4 => a4_init,
            adaptive_on => 0.,
            ...
            pendulum.pulse_a4 => 0., pendulum.pulse_a4_decay_rate => 1.,
            pendulum.freq_event_prediction_error => 0.,
            pendulum.pulse_a3 => 0., pendulum.pulse_a3_decay_rate => 1.,
            pendulum.amp_event_prediction_error => 0.,
            pendulum.amp_event_error_sign => 0.,
            ]
...
pFreqz = plot(sol[2000:end], idxs=[controller.error],legend=:topleft)#,dpi=300, size=(1200,510))
plot!(twinx(),sol, idxs=[first_neur.a4sat],color=:red,xticks=:none,xlabel="",legend=:topright)
        
pAmp = plot(sol, idxs=[controller.amp_error],legend=:topleft)#,dpi=300, size=(1200,510))
plot!(twinx(),sol, idxs=[first_neur.a3sat],color=:red,xticks=:none,xlabel="",legend=:bottomright)
        

data_title = "4_adaptivecontrol_tuning_pulsebased_$experiment_idx"
...
save_string = "figs/$data_title.jld"
